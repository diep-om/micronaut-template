...
INSERT INTO `sequences` (`sequence_name`, currval)
VALUES ('product_uid_seq', 0);
...
DELIMITER $$
CREATE TRIGGER `product_uid_trigger`
  BEFORE INSERT
  ON `product`
  FOR EACH ROW
BEGIN
  ...
  SET NEW.`product_uid` =
          (SELECT `currval` FROM `sequences` WHERE `sequence_name` = 'product_uid_seq');
END $$
DELIMITER ;
